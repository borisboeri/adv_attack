...
def restore_vgg(sess):
    caffe_weights_fn = '/home/gpu_user/assia/ws/tf/caffe-tensorflow/vgg/data.ckpt'
    caffe_weights = np.load(caffe_weights_fn).item()
    var_to_restore = []
    vgg_var_list = [l.split("\n")[0] for l in open('net_vars/vgg_var.txt').readlines()]
...
def restore_alexnet(sess):
    print('Weights initialization ...')
    caffe_weights_fn = '/home/gpu_user/assia/ws/tf/caffe-tensorflow/alexnet/data.ckpt'
    caffe_weights = np.load(caffe_weights_fn).item()
    var_to_restore = []
    nosplit_list = [l.split("\n")[0] for l in open('net_vars/nosplit_var.txt').readlines()]
    split_list = [l.split("\n")[0] for l in open('net_vars/split_var.txt').readlines()]
    var_list =  tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    for var in var_list:
        if len(var.op.name.split("/"))!=3:
            #print('No load %s' %var.op.name)
