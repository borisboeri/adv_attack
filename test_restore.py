...
def train_network(args): 
    ...
    # normal train weights directory
    ...
    with tf.Graph().as_default():
        ...
        original_var_list =  tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='original')
        perturbed_var_list =  tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='perturbed')
        
        #print('\noriginal')
        #for var in original_var_list:
        #    print(var.op.name)
        #    print(var)
        #    raw_input('wait')
        #print('\nperturbed')
        #for var in perturbed_var_list:
        #    print(var.op.name)
        #raw_input('wait')
        ...
        # TODO: filter out adam variables

        # saver for all variables 
        #print('\n')
        #var_to_restore = []
        #var_to_restore = [l.split("\n")[0] for l in open('original_list.txt').readlines()]
        ...
        with tf.Session() as sess:
            ...
            if args.start == 0:
                ...
                print("original checkpoint path: ", ckpt.model_checkpoint_path)
                raw_input('wait')
                var_list = checkpoint_utils.list_variables(original_log_dir)
                #for v in var_list:
                #    print(v)
                ...
                ckpt = tf.train.get_checkpoint_state(train_log_dir) # path to current noisy training log
                print("perturbed checkpoint path: ", ckpt.model_checkpoint_path)
                raw_input('wait')
                #exit(0)
                if ckpt and ckpt.model_checkpoint_path:
            ...
            try:
                ...
                for step in range(min_step, max_step):
                    ...
                    loss_np, acc_np = sess.run([loss_op, acc_op])
                ...
                saver_all.save(sess, checkpoint_path, global_step=step)
