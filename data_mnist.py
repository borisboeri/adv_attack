...
"""Routine for decoding binary file format."""
...
def read_bin(filename_queue):
  ...
  # Dimensions of the images in the mnist dataset.
  label_bytes = 1  
  result.height = 28
  result.width = 28
  result.depth = 1
  ...
  depth_major = tf.reshape(tf.slice(record_bytes, [label_bytes], [image_bytes]), [result.height, result.width, result.depth])
